...
# Swap the rows and columns of product_featues just so it's easier to work with
M = np.transpose(M)
...
# 3. Each movie has several features. Sum those featues to get a total 'difference score' for
total_difference = np.sum(absolute_difference, axis=1)

# 4. Create a new column in the movie list with difference score for each movie
movies_df['difference_score'] = total_difference

# 5. Sort the movie list by the difference score, from least diferrence to most difference
sorted_movie_list = movies_df.sort_values('difference_score')

# 6. Print the result, shwoing the 5 most similar movies to movie_id #1
